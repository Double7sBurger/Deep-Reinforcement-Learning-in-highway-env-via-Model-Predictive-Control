...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:#从这里开始训练
    ...
    def _dump_logs(self) -> None:
        ...
        if len(self.ep_info_buffer) > 0 and len(self.ep_info_buffer[0]) > 0:#在这里增加记录的数据
            self.logger.record("rollout/ep_rew_mean", safe_mean([ep_info["r"]-ep_info["pr"] for ep_info in self.ep_info_buffer]))
            self.logger.record("rollout/ep_len_mean", safe_mean([ep_info["l"] for ep_info in self.ep_info_buffer]))
            self.logger.record("rollout/ep_speed_mean", safe_mean([ep_info["v"] for ep_info in self.ep_info_buffer]))#增加了一个速度
            self.logger.record("rollout/ep_mean_rew_mean", safe_mean([(ep_info["r"]-ep_info["pr"])/ep_info["l"] for ep_info in self.ep_info_buffer]))#增加了一个平均奖励
