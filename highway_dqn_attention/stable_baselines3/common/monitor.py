...
class Monitor(gym.Wrapper):
    ...
    def reset(self, **kwargs) -> GymObs:
        ...
        self.rewards = []
        self.speeds = []  # 在这里初始化速度
        self.pre_reward = []  # 预测的奖励
    ...
    def step(self, action: Union[np.ndarray, int]) -> GymStepReturn:
        ...
        """
        if self.needs_reset:  # 包装了一边env.step，此处增加监视的变量
            raise RuntimeError("Tried to step environment that needs reset")
        observation, reward, done, info = self.env.step(action)
        self.rewards.append(reward)
        self.speeds.append(info["speed"])
        self.pre_reward.append(info.get("pre_reward", 0))  # 默认值是0
        if done:
            self.needs_reset = True
            ep_rew = sum(self.rewards)
            ep_rew_pre = sum(self.pre_reward)
            ep_len = len(self.rewards)
            ep_speed = sum(self.speeds) / len(self.speeds)
            ep_info = {"r": round(ep_rew, 6), "l": ep_len, "t": round(time.time() - self.t_start, 6),
                       "v": round(ep_speed, 6), "pr": round(ep_rew_pre, 6)}
            for key in self.info_keywords:
                ep_info[key] = info[key]
            self.episode_returns.append(ep_rew - ep_rew_pre)  # 只记录真实奖励
            self.episode_lengths.append(ep_len)
            self.episode_speeds.append(ep_speed)
