...
class my_env(gym.Env):
    ...
    def __init__(self, config):
        ...
        self.capacity = 3000
        # 预测的参数
        self.pre_length = 2  # 预测的长度
        self.vision = 1  # 看的是2步之后有没有碰撞
        #
        ...
        self.optimizer = torch.optim.SGD(self.pre_model.parameters(), lr=1e-3, weight_decay=0.01)
        self.loss_target = 0.06  # 目标loss
        self.learn_size = 200
    ...
    def learn(self):
        ...
        sample_number = min(len(indices1), int(self.learn_size / 2))
        selected_indices1 = np.random.choice(indices1, sample_number, replace=False)
        selected_indices2 = np.random.choice(indices2, self.learn_size - sample_number, replace=False)
...
def baseline_train(config):
    ...
    # D:\Anaconda3\envs\pytorch_gpu\Scripts\tensorboard --logdir="E:\python_code\highway_dqn _attention\tensorboard\highway_com/"
    model.learn(total_timesteps=int(8e5), n_eval_episodes=20)
    model.save("model/highway_dqn_rnn/dqn_model_attention8e5")  # 32
