...
class Vehicle(RoadObject):
    ...
    def step(self, dt: float) -> None:
        ...
        """
        self.clip_actions()
        delta_f = self.action['steering']   # delta_f is 方向盘转角，用于在Bicycle Model中计算下面的beta
        beta = np.arctan(1 / 2 * np.tan(delta_f))  # beta is a slip angle，用来修正车辆的朝向，也就是更新车辆的前进方向
        v = self.speed * np.array([np.cos(self.heading + beta),  # 车辆heading+beta，即为修正后的车辆行驶方向,然后分别*cos和sin计算vx,vy
        ...
        if self.impact is not None:
            ...
            self.crashed = True
            self.impact = None
        self.heading += self.speed * np.sin(beta) / (self.LENGTH / 2) * dt  #考虑到车辆的运动学特征，heading不能直接+=beta,而是v/lr*sin(beta)*dt
        self.speed += self.action['acceleration'] * dt
        self.on_state_update()

    def sub_step(self, dt: float, acceleration, steering) -> None:
        # for mpc
        # acceleration是变量
        ...
        # )
        #
        vx = self.speed * cos(self.heading + beta)
        vy = self.speed * sin(self.heading + beta)
        self.position += ca.vertcat(vx * dt, vy * dt)  # 更新车辆坐标
        # self.position += v * dt
        self.heading += self.speed * np.sin(beta) / (self.LENGTH / 2) * dt  # 更新车辆行驶角度
        self.speed += acceleration * dt   #  更新车辆速度
