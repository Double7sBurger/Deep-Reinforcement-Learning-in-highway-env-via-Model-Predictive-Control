...
class OccupancyGridObservation(ObservationType):
    ...
    def __init__(self,
        ...
        grid_shape[0] = grid_shape[0] + 1  # 补充自我车辆的位置
    ...
    def normalize(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for feature, f_range in self.features_range.items():
            if feature in df:
                df[feature] = utils.lmap(df[feature], [f_range[0], f_range[1]], [-1, 1])  # 根据范围映射到-1到1
        return df
    ...
    def observe(self) -> np.ndarray:
        ...
        if self.absolute:
            ...
            df = pd.DataFrame.from_records(
                [v.to_dict(self.observer_vehicle) for v in self.env.road.vehicles])  # 在这里减掉了自我车辆的坐标和速度
            # Normalize
            ...
            for layer, feature in enumerate(self.features):
                if feature in df.columns:  # A vehicle feature
                    for index, vehicle in df.iterrows():
                        ...
                        # if index==0:
                        if 0 <= cell[0] < self.grid.shape[-2] - 1 and 0 <= cell[1] < self.grid.shape[-1]:
                            self.grid[layer, cell[0] + 1, cell[1]] = vehicle[feature]#+np.random.rand(1)
                        # 引入了后面车辆的信息
                        if (cell[0] == -1) and (cell[1] == 0 or cell[1] == 2):
                            self.grid[layer, cell[0] + 1, cell[1]] = vehicle[feature]#+np.random.rand(1)
            ...
            obs[1, 0, 1] = self.observer_vehicle.destination[1] / 100 #y
            obs[2:, 0, 1] = (self.observer_vehicle.velocity / [30, 6]) #x
